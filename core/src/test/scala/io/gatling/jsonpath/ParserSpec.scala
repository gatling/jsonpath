...
class ParserSpec extends FlatSpec with Matchers with ParsingMatchers {
  ...
  it should "work with the root object" in {
    parse(root, "$") should beParsedAs(RootNode)
  }
  ...
  it should "work with some predefined comparison operators" in {
    ...
    parse(subscriptFilter, "[?(@ == 2)]") should beParsedAs(
      ComparisonFilter(EqOperator, SubQuery(List(CurrentNode)), FilterDirectValue(LongValue(2)))
    )
    parse(subscriptFilter, "[?(@==2)]") should beParsedAs(
      ComparisonFilter(EqOperator, SubQuery(List(CurrentNode)), FilterDirectValue(LongValue(2)))
    )
    parse(subscriptFilter, "[?(@ <= 2)]") should beParsedAs(
      ComparisonFilter(LessOrEqOperator, SubQuery(List(CurrentNode)), FilterDirectValue(LongValue(2)))
    )
    parse(subscriptFilter, "[?(@<=2)]") should beParsedAs(
      ComparisonFilter(LessOrEqOperator, SubQuery(List(CurrentNode)), FilterDirectValue(LongValue(2)))
    )
    parse(subscriptFilter, "[?(@ >= 2)]") should beParsedAs(
      ComparisonFilter(GreaterOrEqOperator, SubQuery(List(CurrentNode)), FilterDirectValue(LongValue(2)))
    )
    parse(subscriptFilter, "[?(@>=2)]") should beParsedAs(
      ComparisonFilter(GreaterOrEqOperator, SubQuery(List(CurrentNode)), FilterDirectValue(LongValue(2)))
    )
    parse(subscriptFilter, "[?(@ < 2)]") should beParsedAs(
      ComparisonFilter(LessOperator, SubQuery(List(CurrentNode)), FilterDirectValue(LongValue(2)))
    )
    parse(subscriptFilter, "[?(@<2)]") should beParsedAs(
      ComparisonFilter(LessOperator, SubQuery(List(CurrentNode)), FilterDirectValue(LongValue(2)))
    )
    parse(subscriptFilter, "[?(@ > 2)]") should beParsedAs(
      ComparisonFilter(GreaterOperator, SubQuery(List(CurrentNode)), FilterDirectValue(LongValue(2)))
    )
    parse(subscriptFilter, "[?(@>2)]") should beParsedAs(
      ComparisonFilter(GreaterOperator, SubQuery(List(CurrentNode)), FilterDirectValue(LongValue(2)))
    )
    parse(subscriptFilter, "[?(@ == true)]") should beParsedAs(
      ComparisonFilter(EqOperator, SubQuery(List(CurrentNode)), FilterDirectValue(BooleanValue(true)))
    )
    parse(subscriptFilter, "[?(@==true)]") should beParsedAs(
      ComparisonFilter(EqOperator, SubQuery(List(CurrentNode)), FilterDirectValue(BooleanValue(true)))
    )
    parse(subscriptFilter, "[?(@ != false)]") should beParsedAs(
      ComparisonFilter(NotEqOperator, SubQuery(List(CurrentNode)), FilterDirectValue(BooleanValue(false)))
    )
    parse(subscriptFilter, "[?(@!=false)]") should beParsedAs(
      ComparisonFilter(NotEqOperator, SubQuery(List(CurrentNode)), FilterDirectValue(BooleanValue(false)))
    )
    parse(subscriptFilter, "[?(@ == null)]") should beParsedAs(
      ComparisonFilter(EqOperator, SubQuery(List(CurrentNode)), FilterDirectValue(NullValue))
    )
    parse(subscriptFilter, "[?(@==null)]") should beParsedAs(
      ComparisonFilter(EqOperator, SubQuery(List(CurrentNode)), FilterDirectValue(NullValue))
    )
    ...
    parse(subscriptFilter, "[?(@.foo == 2)]") should beParsedAs(
      ComparisonFilter(EqOperator, SubQuery(List(CurrentNode, Field("foo"))), FilterDirectValue(LongValue(2)))
    )
    parse(subscriptFilter, "[?(true == @.foo)]") should beParsedAs(
      ComparisonFilter(EqOperator, FilterDirectValue(BooleanValue(true)), SubQuery(List(CurrentNode, Field("foo"))))
    )
    parse(subscriptFilter, "[?(2 == @['foo'])]") should beParsedAs(
      ComparisonFilter(EqOperator, FilterDirectValue(LongValue(2)), SubQuery(List(CurrentNode, Field("foo"))))
    )
    ...
    new Parser().compile("$['points'][?(@['y'] >= 3)].id").get should be(RootNode
      :: Field("points")
      :: ComparisonFilter(GreaterOrEqOperator, SubQuery(List(CurrentNode, Field("y"))), FilterDirectValue(LongValue(3)))
      :: Field("id") :: Nil)

    new Parser().compile("$.points[?(@['id']=='i4')].x").get should be(RootNode
      :: Field("points")
      :: ComparisonFilter(EqOperator, SubQuery(List(CurrentNode, Field("id"))), FilterDirectValue(StringValue("i4")))
      :: Field("x") :: Nil)
  ...
  it should "work with some predefined boolean operators" in {
    ...
    parse(subscriptFilter, "[?(@.foo || @.bar <= 2)]") should beParsedAs(
      BooleanFilter(
        OrOperator,
        HasFilter(SubQuery(List(CurrentNode, Field("foo")))),
        ComparisonFilter(LessOrEqOperator, SubQuery(List(CurrentNode, Field("bar"))), FilterDirectValue(LongValue(2)))
      )
    )
  }
}
...
trait ParsingMatchers {
  ...
  class SuccessBeMatcher[+T <: AstToken](expected: T) extends Matcher[Parser.ParseResult[AstToken]] {
    def apply(result: Parser.ParseResult[AstToken]): MatchResult = result match {
      case Success(res, _) =>
        MatchResult(
          expected == res,

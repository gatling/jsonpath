...
object Parser extends RegexParsers {
  ...
  private def numberValue: Parser[FilterDirectValue] = NumberValueRegex ^^ {
    s =>
      if (s.indexOf('.') != -1) FilterDirectValue(DoubleValue(s.toDouble))
      else FilterDirectValue(LongValue(s.toLong))
  }

  private def booleanValue: Parser[FilterDirectValue] =
    "true" ^^^ FilterDirectValue(BooleanValue(true)) |
      "false" ^^^ FilterDirectValue(BooleanValue(false))

  private def nullValue: Parser[FilterValue] =
    "null" ^^^ FilterDirectValue(NullValue)

  private def stringValue: Parser[FilterDirectValue] = quotedValue ^^ {
    str => FilterDirectValue(StringValue(str))
  }

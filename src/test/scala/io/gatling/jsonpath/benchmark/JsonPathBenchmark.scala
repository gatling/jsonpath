...
object JsonPathBenchmark extends PerformanceTest.Quickbenchmark {
	...
	val goessnerJSmart = JSONValue.parse(goessnerJson)
	val goessnerJackson = mapper.readTree(goessnerJson)
	...
	performance of "Gatling JsonPath" in {
		...
		measure method "query resolving" in {
			using(goessnerQueries) in {
				JsonPath.queryJsonObject(_, goessnerJackson).right.map(_.toVector)
			}
			using(twitterQueries) in {
				JsonPath.queryJsonObject(_, twitterJackson).right.map(_.toVector)
			}
		}

		measure method "Jayway query resolving" in {
			using(goessnerQueries) in {
				q =>
					JaywayJsonPath.compile(q).read(goessnerJSmart)
					Nil

			}
			using(twitterQueries) in {
				q =>
					JaywayJsonPath.compile(q).read(twitterJSmart)
					Nil
		...
		measure method "pre-compiled query resolving" in {
			using(goessnerQueries.map(JsonPath.compile(_))) in {
				_.right.map(_.queryJsonObject(goessnerJackson)).right.map(_.toVector)
			}
			using(twitterQueries.map(JsonPath.compile(_))) in {
				_.right.map(_.queryJsonObject(twitterJackson)).right.map(_.toVector)
			}
		}

		measure method "Jayway pre-compiled query resolving" in {
			using(goessnerQueries.map(JaywayJsonPath.compile(_))) in {
				q =>
					q.read(goessnerJSmart)
					Nil
			}
			using(twitterQueries.map(JaywayJsonPath.compile(_))) in {
				q =>
					q.read(twitterJSmart)
					Nil
			}
		}
	}
